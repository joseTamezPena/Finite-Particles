...
psinl = simplify(dot(r_vec_l1,r_vec_l1)/dot(vec_l1,r_vec_l1)) %[output:93ff2ce7]
psinr = simplify(dot(r_vec_r1,r_vec_r1)/dot(vec_r1,r_vec_r1)) %[output:201b8515]
...
totfInf = simplify(limit(totf,r,Inf),'Criterion','preferReal','Steps',100) % Net force by plate charge density %[output:2f6e7422]
totfInfV0 = limit(totfInf,v,0) %[output:9d1ffdf5]

totfInfRel = simplify(totfInf/totfInfV0) %[output:28fedc56]

totfInfRel = taylor(totfInfRel,v,order=taylorOrder) %[output:24ee61db]
...
figure(1)
plotrange = [0,0.99];
fplot(totfInfRel,plotrange) %[output:83b3e2f2]
hold on %[output:83b3e2f2]
relEq = sqrt(1-v^2) %[output:526cd76b]
relEqApx = taylor(relEq,v,Order=taylorOrder) %[output:5ae8d4fa]
fplot(relEq,plotrange) %[output:83b3e2f2]
fplot(relEqApx,plotrange) %[output:83b3e2f2]
xlabel('v/c')  %[output:83b3e2f2]
ylabel('ratio')  %[output:83b3e2f2]
legend({'Force Carriers',"Lorentz Mass","Lorentz Apx"},'Location','northeast') %[output:83b3e2f2]
hold off %[output:83b3e2f2]
...
%---
%[output:2f6e7422]
%   data: {"dataType":"symbolic","outputData":{"name":"totfInf","value":"\\frac{\\log \\left(v-\\sqrt{-1+v^2 }\\right)+\\log \\left(-\\frac{\\sqrt{-1+v}\\,\\sqrt{1-v}}{1+v}\\right)+\\log \\left(v+\\sqrt{-1+v^2 }\\right)+\\frac{\\pi \\,\\mathrm{i}}{2}}{v}-v\\,{\\left(\\log \\left(-\\frac{\\sqrt{-1+v}\\,\\sqrt{1-v}}{1+v}\\right)+\\frac{\\pi \\,\\mathrm{i}}{2}\\right)}+4"}}
%---
%[output:9d1ffdf5]
%   data: {"dataType":"symbolic","outputData":{"name":"totfInfV0","value":"2"}}
%---
%[output:28fedc56]
%   data: {"dataType":"symbolic","outputData":{"name":"totfInfRel","value":"\\begin{array}{l}\n2+\\frac{\\sigma_2 +\\log \\left(v-\\sqrt{-1+v^2 }\\right)-\\log \\left(1+v\\right)+\\sigma_1 +\\log \\left(v+\\sqrt{-1+v^2 }\\right)+\\frac{\\pi \\,\\mathrm{i}}{2}}{2\\,v}-\\frac{v\\,{\\left(\\sigma_2 -\\log \\left(1+v\\right)+\\sigma_1 +\\frac{\\pi \\,\\mathrm{i}}{2}\\right)}}{2}\\\\\n\\mathrm{}\\\\\n\\textrm{where}\\\\\n\\mathrm{}\\\\\n\\;\\;\\sigma_1 =\\frac{\\log \\left(1-v\\right)}{2}\\\\\n\\mathrm{}\\\\\n\\;\\;\\sigma_2 =\\log \\left(-\\sqrt{-1+v}\\right)\n\\end{array}"}}
%---
%[output:24ee61db]
