...
df_v_one = simplify(subs(dforce,[v,r],[0.9,0]),'Steps',100) % Due to the four point charges %[output:45e059fe]
%%
%[text] ## Integrate all the angles
%dforce_xT = expand(taylor(dforce_x,v,Order=taylorOrder));
dforce_xT = dforce_x;
dcircle = simplify(int(dforce_xT,a,0,pi),'IgnoreAnalyticConstraints',true,'Criterion','preferReal','Steps',100) % From zero to pi i.e., the full circle %[output:6464ecee]
%%
dcircle = simplify(dcircle,steps=100); % Net force of the two full circles with charge density
df_v_zero = simplify(subs(dcircle,[v,r],[0.5,0]),'Steps',100) % Net force of the full zero radius circle (point) %[output:926989fe]
%[text] ## 
%%
%[text] ## Integrate for all values of r

dtotf = simplify(expand(r*dcircle),'IgnoreAnalyticConstraints',true,'Criterion','preferReal','Steps',100)/pi; % Add all the circles 
%dtotf = taylor(dtotf,v,Order=taylorOrder);
totf = int(dtotf,r,0,r);
totf = simplify(totf,'IgnoreAnalyticConstraints',true,'Criterion','preferReal','Steps',100); % The force on a charge due a charge surface density

%%
%[text] ## Set the Plates Radius to Infinity

totfInf = simplify(limit(totf,r,Inf),'IgnoreAnalyticConstraints',true,'Criterion','preferReal','Steps',100) % Net force by plate charge density %[output:733de68c]
totfInfV0 = limit(totfInf,v,0) %[output:7d8515b9]

totfInfRel = simplify(totfInf/totfInfV0) %[output:2d31d5aa]

totfInfRel = taylor(totfInfRel,v,order=taylorOrder) %[output:4f08d0fc]
...
figure(1)
plotrange = [0,0.99];
fplot(totfInfRel,plotrange) %[output:4e37255d]
hold on %[output:4e37255d]
relEq = sqrt(1-v^2) %[output:4a5e14ba]
relEqApx = taylor(relEq,v,Order=taylorOrder) %[output:8cd61783]
fplot(relEq,plotrange) %[output:4e37255d]
fplot(relEqApx,plotrange) %[output:4e37255d]
xlabel('v/c')  %[output:4e37255d]
ylabel('ratio')  %[output:4e37255d]
legend({'Force Carriers',"Lorentz Mass","Lorentz Apx"},'Location','northeast') %[output:4e37255d]
hold off %[output:4e37255d]
...
%---
%[output:6464ecee]
%   data: {"dataType":"symbolic","outputData":{"name":"dcircle","value":"\\frac{4\\,\\pi \\,d}{{{\\left(r^2 +d^2 \\right)}}^{3\/2} }-\\frac{4\\,\\pi \\,d\\,r^2 \\,v^2 }{{\\left(-r^2 +{\\left(-1+v^2 \\right)}\\,d^2 \\right)}\\,{{\\left(r^2 +d^2 \\right)}}^{3\/2} }"}}
%---
%[output:926989fe]
%   data: {"dataType":"symbolic","outputData":{"name":"df_v_zero","value":"\\frac{4\\,\\pi }{d^2 }"}}
%---
%[output:733de68c]
%   data: {"dataType":"symbolic","outputData":{"name":"totfInf","value":"2\\,v\\,{\\left(\\log \\left(1+v\\right)-\\log \\left(1-v\\right)\\right)}-\\frac{2\\,\\log \\left(1+v\\right)-2\\,\\log \\left(1-v\\right)}{v}+8"}}
%---
%[output:7d8515b9]
%   data: {"dataType":"symbolic","outputData":{"name":"totfInfV0","value":"4"}}
%---
%[output:2d31d5aa]
%   data: {"dataType":"symbolic","outputData":{"name":"totfInfRel","value":"\\frac{v\\,{\\left(\\log \\left(1+v\\right)-\\log \\left(1-v\\right)\\right)}}{2}-\\frac{2\\,\\log \\left(1+v\\right)-2\\,\\log \\left(1-v\\right)}{4\\,v}+2"}}
%---
%[output:4f08d0fc]
