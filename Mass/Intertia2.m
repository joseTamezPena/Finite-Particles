...
eq1 = -a/2*t^2+c*t+2*r_o==0 % right corpuscle moving towards the left element %[output:10a3dc79]
t_1=solve(eq1,t) % The time it takes to reach the left element  %[output:72ff6c26] %[output:1e1413c7]
t_1 = t_1(1) %[output:2faa7b62]
eq2 = a/2*t^2+c*t+2*r_o==0 % The equation of the corpuscle traveling from left to right element %[output:52a9556c]
t_2= solve(eq2,t) % The time it takes to reach the right element %[output:20ae3570] %[output:96ef4925]
t_2=t_2(2) % I only take the shortest time %[output:2741eabf]
vpa(subs(t_1,[a,r_o,c],[1,1.0e-15,1]))-vpa(subs(t_2,[a,r_o,c],[1,1.0e-15,1])) %[output:63fd7f08]
D_a=simplify(1/(t_2^2)-1/(t_1^2)) % The force is due to differences in arrival time %[output:2c29268f]
D_a= taylor(D_a,a,Order=4) % Lets take an aproximation %[output:37918899]
F=K*Q^2/(4*c^2)*D_a % The reaction force experimented by the acelerated particle  %[output:9cc0585a]
...
U= -K*Q^2/4*int(1/t^2,t,Inf,2*r_o) % The energy required to bound the two elements of the particle %[output:77b8bb66]
syms U
F_net=expand(simplify(subs(F,Q^2,8*U*r_o/K))) % The reaction force in units of the energy %[output:61bd46dd]
%%
%[text] where $r\_o$ is very small and c is very large. 
%[text] The first term of the total reactive force is:
F_net = taylor(F_net,Order=2) % The first term %[output:3cec20f3]
%[text] 
...
%[text]{"align":"center"} ![](text:image:074e).
...
eq3 = (c*t)^2 - r_o^2 - (r_o-1/2*a*t^2)^2 == 0 % corpuscle moving from off x-axis left %[output:142fea84]
t_dL= solve(eq3,t) % The time it takes for corpuscles to reach diagonal left element  %[output:749b75b8] %[output:710dbc41]
t_dL = t_dL(2) %[output:46495920]
eq4 = (c*t)^2 - r_o^2 - (r_o + 1/2*a*t^2)^2 == 0 % diagnoal corpuscle to right element %[output:007799e2]
t_dR= solve(eq4,t) % Diagonal Time to reach the right element %[output:2dcb4f67] %[output:89e185df]
t_dR = t_dR(2) % The shortest time %[output:96c4229c]

eq5= (1/2*a*t^2)^2-(c*t)^2 + (2*r_o)^2==0 % The equation from off x-axis to the opposite element %[output:96905b3c]
t_5= solve(eq5,t) %[output:1c1dc758] %[output:77d51b43]
t_5=t_5(2) % The shortest time %[output:5d7c0d00]
eq6= (1/2*a*t^2)^2-(c*t)^2 + 2*r_o^2 == 0 % Off axis to the next off axis %[output:0975fd80]
t_6= solve(eq6,t) %[output:049eba6b] %[output:195178a3]
t_6=t_6(2) % The shortest time %[output:79ad2b29]
...
F_net = K*(Q/6)^2*simplify(taylor(F_netp,a,Order=2)) %[output:41feb6a8]
%%
% The Binding energy of the regular octahedron
U_1 = K*(Q/6)^2/(2*r_o) % Right to left %[output:53689769]
U_2 = K*(Q/6)^2/(sqrt(2)*r_o) % off diagonal  %[output:19bd15f2]
U_net = simplify(3*U_1 + 12*U_2) %[output:27f8a22e]
syms U_net Q2
qeU = U_net - (K*Q2*(4*sqrt(sym(2)) + 1))/(24*r_o) == 0 %[output:54dc501f]
simplify(solve (qeU,Q2)) %[output:19d239a1]
F_net %[output:7874f780]
F_u=simplify(subs(F_net,Q^2,(24*U_net*r_o)/(K*(4*sqrt(sym(2)) + 1)))) %[output:2e532389]
vpa(subs(F_u,[a,c,r_o],[a,1,1])) %[output:99245b4f]
...
eq1p = -a/2*t^2-c*t+r_o==0 % right corpuscle moving towards the left element %[output:3af12372]
t_1p=solve(eq1p,t) % The time it takes to reach the left element  %[output:399f3cbf] %[output:0d997952]
t_1p = t_1p(1) %[output:55917c61]
eq2p = -a/2*t^2+c*t-r_o==0 % The equation of the corpuscle traveling from left to right element %[output:01f044ae]
t_2p= solve(eq2p,t) % The time it takes to reach the right element %[output:75ad86fe] %[output:449183d8]
t_2p=t_2p(2) % I only take the shortest time %[output:3b188fa0]
eq_c = (1/2*a*t^2)^2-(c*t)^2 + r_o^2==0 % Points off axis %[output:6c50a3a9]
t_c= solve(eq_c,t) %[output:048a659a] %[output:262efa64]
t_c=t_c(2) % The shortest time %[output:43d5942f]
...
F_netc = K*(Q/7)^2*simplify(taylor(F_netcp,a,Order=2)) %[output:85a69456]
%%
% The Binding energy of the regular octahedron with a central element
U_p = K*(Q/7)^2/(r_o) % Elements pairs separed by r_o %[output:8fcb6eaa]
U_1 = K*(Q/7)^2/(2*r_o) % Elements  pairs separed by the sqrt(2)r_o %[output:744c32a9]
U_2 = K*(Q/7)^2/(sqrt(2)*r_o) % Elements pairs separated by 2r_o %[output:9d9e0fb7]
U_net = simplify(3*U_1 + 12*U_2 + 6*U_p) % The 21 pairs of element to element interactions. %[output:394b4a2f]
syms U_net Q2
qeU = U_net - (3*K*Q2*(4*sqrt(sym(2)) + 5))/(98*r_o) == 0 %[output:36c64351]
simplify(solve (qeU,Q2)) %[output:8375bce1]
F_netc %[output:2e49228d]
F_uc=simplify(subs(F_netc,Q^2,(98*U_net*r_o)/(3*K*(4*sqrt(sym(2)) + 5)))) %[output:35146f12]
vpa(subs(F_uc,[a,c,r_o],[a,1,1])) %[output:23684c94]
...
%---
%[output:41feb6a8]
%   data: {"dataType":"symbolic","outputData":{"name":"F_net","value":"-\\frac{K\\,Q^2 \\,a\\,{\\left(2\\,\\sqrt{2}+1\\right)}}{36\\,c^2 \\,r_o }"}}
%---
%[output:53689769]
...
%---
%[output:2e532389]
%   data: {"dataType":"symbolic","outputData":{"name":"F_u","value":"-\\frac{U_{\\textrm{net}} \\,a\\,{\\left(\\frac{4\\,\\sqrt{2}}{93}+\\frac{10}{31}\\right)}}{c^2 }"}}
%---
%[output:99245b4f]
%   data: {"dataType":"symbolic","outputData":{"name":"ans","value":"-0.38340703494077828166888983760042\\,U_{\\textrm{net}} \\,a"}}
%---
%[output:3af12372]
...
%---
%[output:0d997952]
%   data: {"dataType":"symbolic","outputData":{"name":"t_1p","value":"\\left(\\begin{array}{c}\n-\\frac{c+\\sqrt{c^2 +2\\,a\\,r_o }}{a}\\\\\n-\\frac{c-\\sqrt{c^2 +2\\,a\\,r_o }}{a}\n\\end{array}\\right)"}}
%---
%[output:55917c61]
...
%---
%[output:85a69456]
%   data: {"dataType":"symbolic","outputData":{"name":"F_netc","value":"-\\frac{K\\,Q^2 \\,{\\left(3\\,a\\,r_o -2\\,c^2 +2\\,\\sqrt{2}\\,a\\,r_o \\right)}}{49\\,c^2 \\,{r_o }^2 }"}}
%---
%[output:8fcb6eaa]
...
%---
%[output:35146f12]
%   data: {"dataType":"symbolic","outputData":{"name":"F_uc","value":"-\\frac{U_{\\textrm{net}} \\,{\\left(\\frac{8\\,\\sqrt{2}}{21}-\\frac{10}{21}\\right)}\\,{\\left(3\\,a\\,r_o -2\\,c^2 +2\\,\\sqrt{2}\\,a\\,r_o \\right)}}{c^2 \\,r_o }"}}
%---
%[output:23684c94]
%   data: {"dataType":"symbolic","outputData":{"name":"ans","value":"-0.06255754757070287573397665684179\\,U_{\\textrm{net}} \\,{\\left(-2.0+5.8284271247461900976033774484194\\,a\\right)}"}}
%---
