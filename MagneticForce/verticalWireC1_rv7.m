...
p2 = simplify(dot(c_s,c_s),'Criterion','preferReal','Steps',100); % density and velocity correction
p3 = 1.0/dot(c_s,c_p); % Reference Speed
p4 = simplify((dot(vp,c_p)*v_w - dot(vp,v_w)*c_p),'Criterion','preferReal','Steps',100);
F_a = k_e*simplify(p1*p2*p3*(c_p+p4),'Criterion','preferReal','Steps',100);
