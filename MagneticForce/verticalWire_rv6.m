...
p2 = simplify(v_rel1m/c^2,'Criterion','preferReal','Steps',100); % Density correction for distance from origen
p3 = simplify((c/dot(v_rel1,ov)),'Criterion','preferReal','Steps',100); % Density correction for distance from origen
...
limit(Ft_c,L,0) %[output:91ee106f]
%Set L to Infinity
Ft_cInf = simplify(limit(Ft_c,L,Inf),'Criterion','preferReal','Steps',100);
%[text] ## The net force
F_atot = k_e*simplify(Ft_cInf - Ft_nInf,'Criterion','preferReal','Steps',100) %[output:304fb044]
