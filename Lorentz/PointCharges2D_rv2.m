...
p2 = simplify(subs((rvel2_s/c^2)*(c/dots),[v_x,y],[0,0]),'Criterion','preferReal','Steps',100); % Density correction due to velocity
...
st_force = simplify(subs(F2_c,v_y,0),'Criterion','preferReal','Steps',100) %[output:518e543c]
st_forceT = simplify(taylor(st_force,vp_y,0,order=2));
st_forceT = simplify(taylor(st_forceT,vp_x,0,order=2)) %[output:8ec8d066]
...
F2T_mnet = simplify(taylor(F2T_mnet,vp_y,0,order=2),'Criterion','preferReal','Steps',100),Ft_neutral %[output:6bbdd9e9] %[output:3281aa82]
