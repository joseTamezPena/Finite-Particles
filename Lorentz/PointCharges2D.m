...
%p3 = (c*dpmp_vs)^2;
E=[q^2/(4*epsilon0*x^2*sym(pi)), sym(0), sym(0)],M=[(q^2*v_y*vp_y)/(2*c^2*epsilon0*x^2*sym(pi)), -(q^2*v_y)/(4*c*epsilon0*x^2*sym(pi)), sym(0)] %[output:7084455b] %[output:01f790b6]
%p3 = (cmag*dpmp_vs)^2;
E=[(q^2*(c - 2*vp_x))/(4*c*epsilon0*x^2*sym(pi)), sym(0), sym(0)],M=[(q^2*v_y*vp_y)/(2*c^2*epsilon0*x^2*sym(pi)), -(q^2*v_y*(c - 2*vp_x))/(4*c^2*epsilon0*x^2*sym(pi)), sym(0)] %[output:938e302d] %[output:4dd4cdfc]
%p3 = c^2/dpmp_n;
E=[(q^2*(c + 2*vp_x))/(4*c*epsilon0*x^2*sym(pi)), sym(0), sym(0)],M=[-(q^2*v_y*vp_y)/(4*c^2*epsilon0*x^2*sym(pi)), -(q^2*v_y*(c + 2*vp_x))/(4*c^2*epsilon0*x^2*sym(pi)), sym(0)] %[output:9872d5fc] %[output:297fa74b]
%p3 = cmag^2/dpmp_n;
E=[q^2/(4*epsilon0*x^2*sym(pi)), sym(0), sym(0)],M=[-(q^2*v_y*vp_y)/(4*c^2*epsilon0*x^2*sym(pi)), -(q^2*v_y)/(4*c*epsilon0*x^2*sym(pi)), sym(0)] %[output:19ca3cf3] %[output:6d4ce54c]
p3 = (c/dpmp_r)*((cmag^2/vsmag)^2);
E=[(q^2*(c - vp_x))/(4*c*epsilon0*x^2*sym(pi)), sym(0), sym(0)],M=[-(q^2*v_y*vp_y)/(4*c^2*epsilon0*x^2*sym(pi)), -(q^2*v_y*(c - vp_x))/(4*c^2*epsilon0*x^2*sym(pi)), sym(0)] %[output:99dfa0db] %[output:21bf5f31]

%%
%[text] ## Combinations

%p3=(cmag^2-c^2)*dpmp_vs^2/2 ;
E=[-(q^2*vp_x)/(4*c*epsilon0*x^2*sym(pi)), sym(0), sym(0)],M=[sym(0), (q^2*v_y*vp_x)/(4*c^2*epsilon0*x^2*sym(pi)), sym(0)] %[output:65baab7d] %[output:25bf36d8]
%p3=c^2/dpmp_n + (cmag^2-c^2)*dpmp_vs^2/2;
E=[(q^2*(c + vp_x))/(4*c*epsilon0*x^2*sym(pi)), sym(0), sym(0)],M=[-(q^2*v_y*vp_y)/(4*c^2*epsilon0*x^2*sym(pi)), -(q^2*v_y*(c + vp_x))/(4*c^2*epsilon0*x^2*sym(pi)), sym(0)] %[output:95241631] %[output:0107409c]
%p3=cmag^2/dpmp_n + (cmag^2-c^2)*dpmp_vs^2/2;
%p3=cmag^2*(1/dpmp_n + dpmp_vs^2/2)-c^2*dpmp_vs^2/2;
E=[(q^2*(c - vp_x))/(4*c*epsilon0*x^2*sym(pi)), sym(0), sym(0)],M=[-(q^2*v_y*vp_y)/(4*c^2*epsilon0*x^2*sym(pi)), -(q^2*v_y*(c - vp_x))/(4*c^2*epsilon0*x^2*sym(pi)), sym(0)] %[output:9287f50a] %[output:53f6d527]

%%
eforce = simplify(subs((k/pvec2)*p2*p3*ov0,[v_x,v_y,y],[0,0,0]),'IgnoreAnalyticConstraints',true,'Criterion','preferReal','Steps',100);
eforce = simplify(taylor(eforce,[vp_x,vp_y],[0,0],order=2)) %[output:801097f6]
...
F2T_net = simplify(taylor(F2T_c,[vp_x,vp_y],[0,0],order=2)) %[output:9d15ab3a]
% The net magnetic Force due to corpuscles
F2T_m=simplify(F2T_net-eforce) %[output:7f0581fc]


%%
% The Lorentz magnetic forces
Ft_neutral,ft_magnetic %[output:69dcf12a] %[output:591259f3]
